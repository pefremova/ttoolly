...
class CustomInfoClass(_TestInfo):
    def __init__(self, test_result, test_method, outcome=_TestInfo.SUCCESS, err=None, subTest=None):
        self.test_method = test_method
        super(CustomInfoClass, self).__init__(test_result, test_method, outcome=outcome, err=err, subTest=subTest)
...
class CustomHtmlTestResult(HtmlTestResult):
    ...
    def generate_index(self, reports_path_list, testRunner):
        ...
        index_file_fullpath = os.path.join(
            report_save_dir, getattr(settings, 'TEST_HTML_REPORT_INDEX_NAME', 'index.html')
        )
    ...
    def generate_reports(self, testRunner):
        ...
        for testcase_class_name, all_tests in all_results.items():
            ...
            for_index_data.append(
                [path_file.replace(os.getcwd() + '/reports/', ''), testRunner.report_name, tests_count, headers]
            )
    ...
    def _report_testcase(self, testCase, test_cases_list):
        ...
        if class_doc:
            full_name += '<br>'.join(
                [line.strip() for line in to_bytes(class_doc).decode('utf-8').strip('<br> ').splitlines()]
            )
