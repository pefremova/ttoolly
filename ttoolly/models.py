        ...
        def decorate(cls, bases, dct):
            for attr in cls.__dict__:
                if (
                    ...
                    and 'check_errors'
                    not in [
                        getattr(d, '__name__', d.__class__.__name__)
...
class GlobalTestMixIn(with_metaclass(MetaCheckFailures, object)):
    ...
    def assert_object_fields(self, obj, params, exclude=None, other_values=None):
        ...
        object_related_field_names = [
            name
            for name in object_fields
            if fields_map[name].__class__.__name__
            in (
                'RelatedObject',
                'ManyToOneRel',
                'OneToOneField',
        ...
        object_one_to_one_field_names = [
            ...
            if fields_map[name].__class__.__name__ == 'OneToOneField'
        ...
        form_to_object_map = {
            ...
            for name in object_related_field_names
            if hasattr(fields_map[name], 'get_accessor_name')
        }
        ...
        for field in fields_for_check:
            ...
            if (
                name_on_form in viewkeys(form_to_field_map)
                or name_in_field in viewkeys(field_to_form_map)
                and (
                ...
                if value is not None:
                    if count_for_check is not None and value.__class__.__name__ in (
                        'RelatedManager',
                        'ManyRelatedManager',
                    ):
                        try:
                    ...
                    for i, el in enumerate(
                        ...
                        _params = dict(
                            [
                                (k.replace('%s-%d-' % (name_on_form, i), ''), params[k])
    ...
    def assert_xpath_count_in_html(self, html, path, count, msg=None):
        ...
        error_message = 'Found %s instances of \'%s\' (Should be %s)' % (
            real_count,
            path,
            count,
        )
    ...
    def get_error_message(self, message_type, field, *args, **kwargs):
        ...
        ERROR_MESSAGES = {
            ...
            'not_exist': '{verbose_obj} с {verbose_field} "{value}" не существует. Возможно оно было удалено?'
            if previous_locals.get('value', '') == ''
            else '{verbose_obj} с {verbose_pk} "{value}" не существует. Возможно оно было удалено?'.format(
                **previous_locals
            ),
        ...
        custom_errors = self.deepcopy(
            getattr(self, 'custom_error_messages', {}).get(
                field if not isinstance(field, (list, tuple)) else tuple(field), {}
            )
        )
        ...
        if (
            message_type in ('max_length_int', 'max_length_digital', 'max_length_file')
        ...
        if (
            message_type in ('min_length_int', 'min_length_digital', 'min_length_file')
        ...
        if field is None:
            return (
                [el.format(**previous_locals) for el in error_message]
    ...
    def get_field_by_name(self, model, field):
        if re.findall(r'[\w_]+\-\d+\-[\w_]+', field):
            ...
            model = getattr(
                related,
                'related_model',
                getattr(getattr(related, 'rel', None), 'to', related.model),
            )
            field = field.split('-')[-1]
    ...
    def _get_field_value_by_name(self, obj, field):
        ...
        if re.findall(r'[\w_]+\-\d+\-[\w_]+', field):
            ...
            qs = (
                getattr(obj, related_names_map.get(model_name, model_name))
                .all()
                .order_by(getattr(self, 'inline_objects_ordering', {}).get(field, 'pk'))
            )
    ...
    def get_params_according_to_type(self, value, params_value):
        ...
        if isinstance(value, bool):
            ...
            return value, params_value
        if (isinstance(value, date) or isinstance(value, time)) and not (
            isinstance(params_value, date) or isinstance(params_value, time)
        ):
    ...
    def get_random_file(self, field, length=10, count=1, *args, **kwargs):
        ...
        file_dict = self.deepcopy(
            getattr(self, 'file_fields_params', {}).get(field, {})
            or getattr(self, 'file_fields_params_add', {}).get(field, {})
            or getattr(self, 'file_fields_params_edit', {}).get(field, {})
        )
    ...
    def get_related_names(self, model):
        ...
        obj_related_objects = dict(
            [
                (
                    el.get_accessor_name(),
                    getattr(el, 'var_name', el.get_accessor_name()),
                )
                for el in get_all_related_objects(model)
            ]
        )
    ...
    def get_related_field_name(self, field):
        ...
        return (
            ...
            and field.related_query_name()
            or getattr(field, 'remote_field', None)
            and field.remote_field.name
            or field.field.name
        )
    ...
    def get_value_for_compare(self, obj, field):
        ...
        if value.__class__.__name__ in (
            ...
            value = [
                v
                for v in value.values_list('pk', flat=True).order_by(
    ...
    def get_value_for_field(self, length, field_name):
        ...
        if self.is_email_field(field_name):
            length = (
                ...
                if length is not None
                    ...
                    getattr(self, 'min_fields_length', {}).get(field_name, 6),
        ...
        if self.is_file_field(field_name):
            if length is None:
                length = max(
                    getattr(self, 'max_fields_length', {}).get(field_name, 1),
                    max(
                        [
                            1,
                        ]
        ...
        length = (
            ...
            if length is not None
                ...
                getattr(self, 'min_fields_length', {}).get(field_name, 1),
    ...
    def is_digital_field(self, field):
        ...
        return (
            ...
            or (field in (getattr(self, 'digital_fields_edit', ()) or ()))
        )
    ...
    def is_email_field(self, field):
        ...
        return (
            'email' in field
            and [
                getattr(self, 'email_fields', None),
                getattr(self, 'email_fields_add', None),
                getattr(self, 'email_fields_edit', None),
            ]
            == [None, None, None]
        ) or (
            (field in (getattr(self, 'email_fields', ()) or ()))
            or (field in (getattr(self, 'email_fields_add', ()) or ()))
            or (field in (getattr(self, 'email_fields_edit', ()) or ()))
        )
    ...
    def is_int_field(self, field):
        ...
        return (
            ...
            or (field in (getattr(self, 'int_fields_edit', ()) or ()))
        )
    ...
    def is_multiselect_field(self, field):
        ...
        return (
            ...
            or (field in (getattr(self, 'multiselect_fields_edit', ()) or ()))
        )
    ...
    def update_params(self, params):
        ...
        for key in set(viewkeys(params)).intersection(unique_keys):
            default_value = params[key] or (
                ...
                or getattr(self, 'default_params_add', {})
                or getattr(self, 'default_params_edit', {})
                or {}
            ).get(key, None)
...
class FormCommonMixIn(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.with_captcha is None:
            self.with_captcha = (
                (self.all_fields and 'captcha' in self.all_fields)
    ...
    def _prepare_choice_fields(self):
        self.choice_fields_add = self.choice_fields_add or list(
            set(self.choice_fields).intersection(self.all_fields_add)
        )
        self.choice_fields_edit = self.choice_fields_edit or list(
            ...
            if self.multiselect_fields is not None:
                ...
                self.multiselect_fields_add = set(
                    [
                        k
                        for k, v in viewitems(self.default_params_add)
                        if 'FORMS' not in k and isinstance(v, (list, tuple))
                    ]
                )
    ...
    def _prepare_file_fields_params(self):
        ...
        if self.file_fields_params_add is None:
            ...
            if not self.file_fields_params_add:
                self.file_fields_params_add = {
                    k: {}
                    for k in set(viewkeys(self.default_params_add))
                    .intersection(('file', 'filename', 'image', 'preview'))
                    .difference(getattr(self, 'not_file', []))
                }
                self.file_fields_params_add.update(
                    {
                        k: {'extensions': ('jpg', 'jpeg', 'png')}
                        for k in set(viewkeys(self.default_params_add))
                        .intersection(('image', 'preview', 'photo'))
                        .difference(getattr(self, 'not_file', []))
                    }
                )
        ...
        if self.file_fields_params_edit is None:
            ...
            if not self.file_fields_params_edit:
                self.file_fields_params_edit = {
                    k: {}
                    for k in set(viewkeys(self.default_params_edit))
                    .intersection(('file', 'filename', 'image', 'preview'))
                    .difference(getattr(self, 'not_file', []))
                }
                self.file_fields_params_edit.update(
                    {
                        k: {'extensions': ('jpg', 'jpeg', 'png')}
                        for k in set(viewkeys(self.default_params_edit))
                        .intersection(('image', 'preview', 'photo'))
            ...
            for value in values:
                ...
                for f_name in self.get_object_fields(value):
                    ...
                    if 'AutoField' in mro_names:
                        continue
                    if (
                        mro_names.intersection(
                            [
                                'ForeignKey',
                            ]
                        )
                        and f.related_model == obj.__class__
                    ):
                        params[f_name] = obj
                        ...
                        if getattr(self, 'choice_fields_values', {}).get(set_name + '-0-' + f_name, ''):
                            ...
                            params[f_name] = (
                                choice(f.related_model._base_manager.all())
                ...
                if subfields:
                    for subfield in subfields:
                        params[subfield] = self.get_params_according_to_type(
                            self._get_field_value_by_name(obj, subfield), ''
                        )[0]
                else:
            ...
            for lead_params in lead_params_list:
                if all((params.get(k, None) == v for k, v in viewitems(lead_params))):
                    ...
                    self.fill_with_related(
                        params,
                        depended_field,
                        self.get_value_for_field(None, depended_field),
                    )
    ...
    def get_all_not_str_fields(self, additional=''):
        ...
        for field_type_name in (
            'digital_fields%s' % additional,
            'date_fields',
            'datetime_fields',
            'choice_fields%s' % additional,
            'choice_fields%s_with_value_in_error' % additional,
            'disabled_fields%s' % additional,
            'hidden_fields%s' % additional,
            'int_fields%s' % additional,
            'multiselect_fields%s' % additional,
            'not_str_fields',
        ):
        ...
        for k, v in viewitems(required_if):
            ...
            for vv in v if isinstance(v, (list, tuple)) else (v,):
                if isinstance(vv, (list, tuple)):
                    ...
                    all_dependent += (vv,)
        return {'lead': all_lead, 'dependent': all_dependent, 'related': related}
...
class UserPermissionsTestMixIn(GlobalTestMixIn, LoginMixIn):
    ...
    def test_unallowed_links_with_404_response(self):
        ...
        """
        links_other = (
            tuple(self.allowed_links)
            + tuple(self.links_403)
            + tuple(self.links_redirect)
            + tuple(self.links_400)
            + tuple(self.links_401)
            + tuple(self.links_405)
        )
...
class ChangePasswordCommonMixIn(object):
    ...
    password_positive_values = [
        get_randname(10, 'w') + str(randint(0, 9)),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.password_params = (
            ...
            or {
                k: v
                for k, v in {
                    self.field_old_password: self.current_password,
    ...
    def check_blacklist_on_positive(self):
        if self.blacklist_model:
            self.assertEqual(
                ...
                0,
                '%s blacklist objects created after valid login'
                % self.blacklist_model.objects.filter(host='127.0.0.1').count(),
            )
...
class LoginCommonMixIn(BlacklistMixIn):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.passwords_for_check = self.passwords_for_check or [
            self.password,
        ]
    ...
    def check_response_on_positive(self, response):
        if self.url_redirect_to:
            ...
            self.assertEqual(
                response.status_code,
                200,
                "Final response code was %d (expected 200)" % response.status_code,
            )
        else:
...
class LoginTestMixIn(LoginCommonMixIn, LoginPositiveCases, LoginNegativeCases):
...
class CustomTestCase(GlobalTestMixIn, TransactionTestCase):
    ...
    def get_model(self, table_name, db_name=None):
        ...
        params.update(
            dict(
                [
                    (
                        k,
                        models.Field(primary_key=True, unique=True)
                        if k != 'id'
                        else models.AutoField(
                            primary_key=True,
                        ),
                    )
                    for k in pk_names
        ...
        params.update(
            {
                'Meta': Meta,
                '__module__': CustomModel.__module__,
